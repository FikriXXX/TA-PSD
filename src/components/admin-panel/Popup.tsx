...
const Popup = ({setOpenPopup, setUpdateTable}: PropsType) => {
    ...
    const handleSubmit = (e: FormEvent) => {
        ...
        axios.put('/api/edit_product/${product_Data._id}', inputData).then(res => {
          makeToast("Product Upadted Succesfully!")
  ...
  return (
    <div className='fixed top-0 left-0 w-full h-screen bg-[#00000070]
    grid place-items-center'>
      <div className='bg-white w-[700px] py-8 rounded-lg text-center relative'>
        ...
        <form className='mt-6 w-fit space-y-4 mx-auto' onSubmit={handleSubmit}>
          <input 
          className='border block border-gray-500 outline-none px-4 py-2 rounder-lg w-fit'
          type="text" 
          placeholder='Name'
          ...
          <div className='flex justify-end'>
            <button className='bg-accent block text-white px-8 py-2 rounder-lg self-center'>
              Save
            </button>
          </div>
        </form>
